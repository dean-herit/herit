...
describe("BeneficiaryCard Component", () => {
  ...
  it("handles long text gracefully", () => {
    const longTextBeneficiary = {
      ...mockBeneficiary,
      name: "This Is A Very Long Name That Should Handle Text Wrapping Properly Without Breaking Layout",
      conditions:
        "This is a very long condition text that should be displayed properly without breaking the card layout and should wrap appropriately",
    };
    ...
    cy.contains("This Is A Very Long Name").should("be.visible");
    cy.contains("This is a very long condition").should("be.visible");

    // Card should maintain reasonable width
    cy.get(`[data-testid="beneficiary-card-${longTextBeneficiary.id}"]`)
      .should("be.visible")
      .and(($card) => {
        expect($card[0].scrollWidth).to.be.lessThan(600); // Reasonable max width
      });
  });
