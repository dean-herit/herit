        ...
        if (devButton) {
            ...
            setTimeout(() => {
                ...
                switches.forEach((sw, index) => {
                    ...
                    if (parentText.includes('Visual Component Mode') || parentText.includes('Visual')) {
                        console.log(`✓ Found Visual Component Mode switch at index ${index}`);
                        sw.click();
                        console.log('✓ Toggled Visual Component Mode');
                        
                        // Test component detection after enabling
                        setTimeout(() => {
                            const componentsWithIds = document.querySelectorAll('[data-component-id]');
                            console.log(`📊 Found ${componentsWithIds.length} components with data-component-id`);
            ...
            Array.from(componentsWithIds).slice(0, 5).forEach((comp, i) => {
                console.log(`  ${i+1}. ${comp.getAttribute('data-component-id')} (${comp.getAttribute('data-component-category') || 'no-category'})`);
            });
        }
    } catch (error) {
...
console.log('  highlightingEnabled:', localStorage.getItem('highlightingEnabled'));
...
setTimeout(() => {
    ...
    if (allComponents.length > 0) {
        ...
        console.log(`✨ Applied temporary highlighting to ${allComponents.length} components`);
    }
}, 3000);
