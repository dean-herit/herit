...
const createMockAuthService = (options: {
  ...
  getCurrentUser: cy.stub().as('getCurrentUser').resolves(
    options.shouldThrow 
...
const createMockApiService = (options: {
  shouldThrow?: boolean;
  response?: any;
} = {}): ApiService => ({
  post: cy.stub().as('apiPost').resolves(
    options.shouldThrow
      ? Promise.reject(new Error('API error'))
      : options.response || { id: '123', name: 'Updated User' }
  ),
  get: cy.stub().as('apiGet').resolves(
    options.response || { data: 'test' }
  ),
});
...
const createMockStorageService = (): StorageService => {
  ...
  return {
    setItem: cy.stub().as('storageSetItem').callsFake((key: string, value: string) => {
      storage.set(key, value);
    }),
    getItem: cy.stub().as('storageGetItem').callsFake((key: string) => {
      return storage.get(key) || null;
    }),
    removeItem: cy.stub().as('storageRemoveItem').callsFake((key: string) => {
      storage.delete(key);
    }),
  };
};
...
describe("UserProfile with Dependency Injection", () => {
  ...
  describe("Successful User Loading", () => {
    it("should load and display user profile", async () => {
      ...
      cy.get('@storageSetItem').should('have.been.calledWith', 'lastUser', mockUser.name);
    });
  });
  ...
  describe("Error States", () => {
    it("should handle auth service errors", () => {
      ...
      cy.get('[data-testid="error-message"]').should('contain', 'Failed to load user profile');
    ...
    it("should handle logout errors", async () => {
      ...
      // Should show error
      cy.get('[data-testid="error-message"]').should('contain', 'Logout failed');
  ...
  describe("Profile Updates", () => {
    it("should update user profile via API", async () => {
      ...
      cy.get('@storageSetItem').should('have.been.calledWith', 'lastUser', 'Jane Doe');
    });
  ...
  describe("No User State", () => {
    it("should handle no user scenario", () => {
      ...
      cy.get('[data-testid="no-user-state"]').should('be.visible');
      cy.get('[data-testid="no-user-state"]').should('contain', 'Please log in');
    });
  });
  ...
  describe("Loading States", () => {
    it("should show loading during operations", () => {
      ...
      const authService: AuthService = {
        getCurrentUser: cy.stub().as('getCurrentUser').returns(
          new Promise(resolve => {
  ...
  describe("Retry Functionality", () => {
    it("should allow retry after error", () => {
      ...
      const authService: AuthService = {
        getCurrentUser: cy.stub().as('getCurrentUser').callsFake(() => {
          if (shouldThrow) {
...
describe("useUserData Hook", () => {
  it("should load user data", () => {
    ...
    cy.get('@storageSetItem').should('have.been.calledWith', 
      'user-cache', 
  ...
  it("should fall back to cached data on error", () => {
    ...
    const storageService: StorageService = {
      ...
      getItem: cy.stub().as('storageGetItem').returns(JSON.stringify(cachedUser)),
    };
