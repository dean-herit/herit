...
describe("DocumentManager", () => {
  ...
  describe("Core Functionality", () => {
    it("renders without crashing", { timeout: 5000, retries: 2 }, () => {
      cy.mountWithContext(
        <QueryClientProvider client={queryClient}>
          <DocumentManager {...mockProps} />
        </QueryClientProvider>,
      );
      cy.get("body").then(() => {
        // Component may render different elements based on props/state
        cy.get(
          '[data-testid="document-button"], [data-testid="documentmanager"]',
        )
          .should("exist")
          .then(($els) => {
            if ($els.length > 0) {
    ...
    it("responds to user interactions", { timeout: 5000, retries: 2 }, () => {
      ...
      cy.get('[data-testid="document-button"]').click();
      cy.get("body").then(() => {
        // Try specific test ID first, fallback to component elements
        cy.get('[data-testid="document-button"], button, div, span, svg')
          .first()
          .should("exist");
      });
    });
