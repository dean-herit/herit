...
function GoogleSignInButtonForTesting({
  ...
  return (
    <div data-testid="google-signin-container" className="w-full max-w-md mx-auto">
      {error && (
        <div 
...
describe("GoogleSignInButton Component", () => {
  ...
  describe("Core Functionality", () => {
    it("renders button elements correctly", () => {
      ...
      cy.get('[data-testid="google-signin-button"]').should("contain", "Continue with Google");
      cy.get('[data-testid="google-signin-button"]').should("have.class", "w-full");
    ...
    it("triggers OAuth action on click", () => {
      cy.mount(
        <TestWrapper>
          <GoogleSignInButtonForTesting onPress={callbacks.onPress} />
        </TestWrapper>
      );
    ...
    it("shows loading state correctly", () => {
      ...
      cy.get('[data-testid="google-signin-button"]').should("have.attr", "data-loading", "true");
      cy.get('[data-testid="google-signin-button"]').should("contain", "Signing in...");
    ...
    it("handles disabled state correctly", () => {
      cy.mount(
        <TestWrapper>
          <GoogleSignInButtonForTesting disabled={true} onPress={callbacks.onPress} />
        </TestWrapper>
      );
  ...
  describe("Error States", () => {
    it("displays error messages", () => {
      ...
      cy.get('[data-testid="google-signin-error"]').should("contain", errorMessage);
    });
    ...
    it("handles OAuth popup blocked errors", () => {
      ...
      cy.get('[data-testid="google-signin-error"]').should("contain", "Popup blocked");
    });
    ...
    it("handles network connectivity issues", () => {
      ...
      cy.get('[data-testid="google-signin-error"]').should("contain", "Network error");
    });
    ...
    it("handles OAuth server errors", () => {
      ...
      cy.get('[data-testid="google-signin-error"]').should("contain", "OAuth service");
    });
    ...
    it("handles user cancellation gracefully", () => {
      ...
      cy.get('[data-testid="google-signin-error"]').should("contain", "cancelled by user");
    });
  });
  ...
  describe("Accessibility", () => {
    ...
    it("supports keyboard navigation", () => {
      ...
      cy.get('[data-testid="google-signin-button"]').focus().should("be.focused");
      cy.realPress("Enter");
      cy.get("@onPress").should("have.been.called");
    ...
    it("has proper ARIA labels and attributes", () => {
      ...
      cy.get('[data-testid="google-signin-button"]').should("have.prop", "tagName", "BUTTON");
    ...
    it("maintains accessibility when disabled", () => {
      ...
      cy.get('[data-testid="google-signin-button"]').should("be.disabled");
      cy.get('[data-testid="google-signin-button"]').should("have.attr", "aria-disabled", "true");
    });
  });

  describe("Performance", () => {
    it("should render quickly", () => {
      TestUtils.measureRenderTime('[data-testid="google-signin-container"]', 1000);
    ...
    it("should handle state changes efficiently", () => {
      const TestStateChanges = () => {
        ...
        return (
          <div>
            <GoogleSignInButtonForTesting isLoading={loading} error={error} />
            <button
  ...
  describe("Edge Cases", () => {
    ...
    it("handles rapid state transitions", () => {
      const TestRapidStates = () => {
        const [state, setState] = useState<"normal" | "loading" | "disabled" | "error">("normal");
        
        React.useEffect(() => {
          const interval = setInterval(() => {
            setState(prev => {
              switch (prev) {
                case "normal": return "loading";
                case "loading": return "disabled";
    ...
    it("handles component remounting", () => {
      const TestMountWrapper = ({ show }: { show: boolean }) => (
        <TestWrapper>
          {show && <GoogleSignInButtonForTesting />}
        </TestWrapper>
      );
  ...
  describe("Security", () => {
    ...
    it("should handle OAuth popup security restrictions", () => {
      ...
      cy.get('[data-testid="google-signin-error"]').should("contain", "security policy");
    });
    ...
    it("should not expose sensitive OAuth details in errors", () => {
      ...
      cy.get('[data-testid="google-signin-error"]').should("contain", "Authentication failed");
      cy.get('[data-testid="google-signin-error"]').should("not.contain", "token");
      cy.get('[data-testid="google-signin-error"]').should("not.contain", "secret");
      cy.get('[data-testid="google-signin-error"]').should("not.contain", "key");
    });
  });
