  ...
  try {
    ...
    if (devButtons > 0) {
      ...
      if (visualToggle > 0) {
        ...
        console.log(`🔘 Visual mode status: ${isEnabled ? 'enabled' : 'disabled'}`);
      ...
      console.log('ℹ️ Visual dev mode button not found - checking localStorage...');
    ...
    if (components.length > 0) {
      ...
      if (dashboardComponent) {
        try {
          ...
          console.log('✅ Dashboard component successfully identified via registry');
    ...
    console.log(`🛠️ Dev mode available: ${devButtons > 0 ? 'Yes' : 'Via localStorage'}`);
