...
class ComponentTestSuite {
  ...
  async testComponent(component) {
    ...
    try {
      ...
      if (existenceTest.passed) {
        ...
        const accessibilityTest = await this.testComponentAccessibility(
          component.id,
        );
        ...
        console.log(
          `⏭️  Skipping further tests for ${component.id} (not found)`,
        );
  ...
  async testComponentMetadata(componentId) {
    try {
      const metadata =
        await this.testFramework.getComponentMetadata(componentId);
      const hasValidMetadata = metadata.id === componentId && metadata.category;

      console.log(
        `${hasValidMetadata ? "✅" : "❌"} ${componentId}: Metadata ${hasValidMetadata ? "valid" : "invalid"}`,
      );
  ...
  async testComponentResponsive(componentId) {
    try {
      const results = await this.testFramework.testResponsiveComponent(
        componentId,
        [
      ...
      console.log(
        `${allVisible ? "✅" : "⚠️"} ${componentId}: Responsive ${allVisible ? "passed" : "needs review"}`,
      );
  ...
  async testComponentAccessibility(componentId) {
    try {
      const accessibility =
        await this.testFramework.checkAccessibility(componentId);
      const score = Object.values(accessibility).filter(
        (v) => v === true,
      ).length;

      console.log(
        `${score > 2 ? "✅" : "⚠️"} ${componentId}: Accessibility score ${score}/6`,
      );
      return {
        test: "accessibility",
        passed: score > 2,
