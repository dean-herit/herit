...
const devPanel = document.querySelector(
...
const instructionsOverlay = document.querySelector(
  ".component-highlighter-container .fixed.bottom-4.left-4",
);
console.log("Instructions overlay found:", !!instructionsOverlay);
console.log(
  "Instructions text:",
  instructionsOverlay?.textContent || "Not found",
);
...
const componentsWithCategory = document.querySelectorAll(
  "[data-component-category]",
);
console.log(`Components with data-component-id: ${componentsWithId.length}`);
console.log(
  `Components with data-component-category: ${componentsWithCategory.length}`,
);

// List first 10 components
console.log("First 10 components:");
Array.from(componentsWithId)
  .slice(0, 10)
  .forEach((comp, i) => {
    console.log(
      `  ${i + 1}. ${comp.getAttribute("data-component-id")} (${comp.getAttribute("data-component-category") || "no-category"}) - ${comp.tagName}`,
    );
  });
...
function testManualHighlighting() {
  if (componentsWithId.length > 0) {
    ...
    console.log(
      `Testing on: ${testComponent.getAttribute("data-component-id")}`,
    );
