...
async function runDashboardTests() {
  ...
  try {
    ...
    const dashboardMetadata = await testFramework.getComponentMetadata('dashboard-client');
    ...
    const cardResponsiveResults = await testFramework.testResponsiveComponent('dashboard-client');
    console.log('Responsive test results:', cardResponsiveResults.length, 'breakpoints tested');
    
    // Test 4: Test quick action buttons
    console.log('\n📋 Test 4: Quick Action Button Interactions');
    
    // Find and test asset management button
    const assetButton = page.locator('text="Manage Assets"').or(page.locator('text="Add Your First Asset"'));
    ...
    const accessibilityResults = await testFramework.checkAccessibility('dashboard-client');
    ...
    if (await devButton.count() > 0) {
      ...
      const visualToggle = page.locator('text="Visual Component Mode"').locator('..').locator('input');
    ...
    for (const element of componentElements.slice(0, 5)) { // Test first 5 components
