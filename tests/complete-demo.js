  ...
  try {
    ...
    if (currentUrl.includes('/onboarding')) {
      ...
      try {
        ...
        await page.fill('input[name="address.street"]', TEST_USER.address.street);
        await page.fill('input[name="address.city"]', TEST_USER.address.city);
        await page.selectOption('select[name="address.county"]', TEST_USER.address.county);
        await page.fill('input[name="address.eircode"]', TEST_USER.address.eircode);
        ...
        const continueBtn = await page.locator('button').filter({ hasText: /Continue|Next|Submit/i }).first();
      ...
      try {
        ...
        const signatureOptions = await page.locator('button, input[type="radio"]').filter({ hasText: /Type|Draw|Upload/i }).all();
        if (signatureOptions.length > 0) {
          ...
          const signatureInput = page.locator('input[placeholder*="signature"], input[name*="signature"], textarea[placeholder*="signature"]');
          if (await signatureInput.count() > 0) {
            await signatureInput.fill(`${TEST_USER.firstName} ${TEST_USER.lastName}`);
          }
        }
        ...
        const continueBtn = page.locator('button').filter({ hasText: /Continue|Next|Submit/i });
        if (await continueBtn.count() > 0) {
          await continueBtn.click();
          await page.waitForTimeout(2000);
        ...
        const continueBtn = await page.locator('button').filter({ hasText: /Continue|Next|Skip/i }).first();
        if (await continueBtn.count() > 0) {
          await continueBtn.click();
          await page.waitForTimeout(2000);
        }
      ...
      try {
        ...
        const continueBtn = page.locator('button').filter({ hasText: /Continue|Next|Accept|Agree/i });
        if (await continueBtn.count() > 0) {
          await continueBtn.click();
          await page.waitForTimeout(2000);
      ...
      try {
        const skipButton = page.locator('button').filter({ hasText: /Skip|Later|Complete/i });
      ...
      if (page.url().includes('/onboarding')) {
        const completeButtons = await page.locator('button').filter({ hasText: /Complete|Finish|Done|Dashboard/i }).all();
    ...
    components.forEach(comp => {
      console.log(`   • ${comp.id} (${comp.category}) ${comp.visible ? '👁️' : '👻'}`);
    });
    ...
    if (devButtons > 0) {
      ...
      if (components.length > 0) {
        ...
        if (visibleComponent) {
          ...
          const componentElement = page.locator(`[data-component-id="${visibleComponent.id}"]`);
    ...
    if (components.length > 0) {
      ...
      const dashboardComponent = components.find(c => c.id === 'dashboard-client');
      ...
      const responsiveResults = await testFramework.testResponsiveComponent('dashboard-client', [
    ...
    console.log(`🛠️ Visual dev mode: ${devButtons > 0 ? '✅ Available' : '⚠️ Via localStorage'}`);
