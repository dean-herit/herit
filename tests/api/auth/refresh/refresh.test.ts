...
import * as routeHandlers from '@/app/api/auth/refresh/route';
  ...
  describe("Core Functionality", () => {
    ...
    it("handles POST requests with valid data", async () => {
      ...
      const request = createAuthenticatedRequest(url, {
        ...
        body: JSON.stringify({}), // Refresh endpoint doesn't need body data
      }, authContext);

      const response = await routeHandlers.POST(request);
      
      // Refresh token validation may fail with generic test tokens
