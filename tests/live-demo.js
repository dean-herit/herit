  ...
  try {
    ...
    const pageAnalysis = await page.evaluate(() => {
      return {
        ...
        hasComponents: document.querySelectorAll('[data-component-id]').length > 0,
    ...
    console.log(`🎨 Visual Dev Mode: ${pageAnalysis.visualDevMode || 'Not set'}`);
    console.log(`🧩 Components with metadata: ${pageAnalysis.hasComponents ? 'Found' : 'None'}`);
    console.log(`📊 Page elements: ${pageAnalysis.totalElements}`);
    console.log(`📝 Forms: ${pageAnalysis.forms}, Buttons: ${pageAnalysis.buttons}, Inputs: ${pageAnalysis.inputs}`);
    ...
    const fs = require('fs');
    const registryPath = require('path').join(process.cwd(), 'lib/component-registry.ts');
    if (fs.existsSync(registryPath)) {
      const registryContent = fs.readFileSync(registryPath, 'utf8');
      const totalMatch = registryContent.match(/Total components: (\d+)/);
      console.log(`📊 Total components in registry: ${totalMatch ? totalMatch[1] : 'Unknown'}`);
