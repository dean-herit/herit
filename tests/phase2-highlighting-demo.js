...
async function phase2HighlightingDemo() {
  ...
  try {
    ...
    try {
      ...
      console.log('ℹ️ Login failed, user may not exist - this is expected for demo');
    }
    ...
    if (hasDevButton) {
      ...
      const boundaryToggle = page.locator('text="Boundary Overlay"').locator('..').locator('input');
      ...
      const animationToggle = page.locator('text="Animate on Hover"').locator('..').locator('input');
    ...
    console.log(`🧩 Found ${componentElements.length} components with data-component-id`);
    ...
    if (componentElements.length > 0) {
      ...
      for (let i = 0; i < Math.min(3, componentElements.length); i++) {
        try {
          ...
          console.log(`⚠️ Could not hover component ${i + 1}: ${error.message}`);
        }
      }
    }
    ...
    if (componentElements.length > 0) {
      try {
        ...
        const componentId = await firstElement.getAttribute('data-component-id');
        
        console.log(`👆 Clicking component for persistent highlight: ${componentId}`);
    ...
    const performanceMetrics = await page.evaluate(() => {
      ...
      return {
        ...
        componentsFound: document.querySelectorAll('[data-component-id]').length,
    ...
    console.log(`   🌟 CSS Highlight API: ${performanceMetrics.supportsHighlightAPI ? 'Supported' : 'Fallback mode'}`);
