...
async function demoWithAuth() {
  ...
  try {
    ...
    components.forEach((comp) => {
      console.log(
        `  - ${comp.id} (${comp.category}) ${comp.visible ? "👁️ visible" : "👻 hidden"}`,
      );
    });
    ...
    if (devButtons > 0) {
      ...
      if (visualToggle > 0) {
        ...
        console.log(
          `🔘 Visual mode currently: ${isEnabled ? "enabled" : "disabled"}`,
        );
        ...
        if (!isEnabled) {
          ...
          console.log(
            "📸 Screenshot: demo-3-visual-mode.png (visual mode enabled)",
          );
        }
      }
    }
    ...
    if (components.length > 0) {
      ...
      console.log(
        "📸 Screenshot: demo-4-component-hover.png (component hover)",
      );
