...
export const IrishDocumentRequirements: Record<string, DocumentRequirement[]> = {
  ...
  [FinancialAssetType.IRISH_BANK_ACCOUNT]: [
    {
      ...
      description: "Statement from the last 3-6 months showing account details and balance",
      priority: DocumentPriority.REQUIRED,
      category: DocumentCategory.FINANCIAL,
      helpText: "Upload your most recent statement showing account number and balance",
  ...
  [FinancialAssetType.IRISH_CREDIT_UNION]: [
    ...
    },
    {
      ...
      priority: DocumentPriority.RECOMMENDED,
      category: DocumentCategory.LEGAL,
      helpText: "Important for ensuring death benefit is paid to intended beneficiary",
    },
  ],
  ...
  [PropertyAssetType.IRISH_RESIDENTIAL_PROPERTY]: [
    {
      ...
      priority: DocumentPriority.REQUIRED,
      category: DocumentCategory.OWNERSHIP,
      helpText: "Proof of property ownership from Land Registry or Registry of Deeds",
    },
    {
      assetType: PropertyAssetType.IRISH_RESIDENTIAL_PROPERTY,
...
export function getDocumentRequirements(assetType: string): {
  ...
  return {
    required: requirements.filter(r => r.priority === DocumentPriority.REQUIRED),
    recommended: requirements.filter(r => r.priority === DocumentPriority.RECOMMENDED),
    optional: requirements.filter(r => r.priority === DocumentPriority.OPTIONAL),
  };
}

export function getRequiredDocumentCount(assetType: string): number {
  const requirements = IrishDocumentRequirements[assetType] || [];
  return requirements.filter(r => r.priority === DocumentPriority.REQUIRED).length;
}
...
export function getDocumentCompleteness(
  ...
  return {
    percentage: requiredCount > 0 ? (uploadedRequired / requiredCount) * 100 : 100,
    required: requiredCount,
    uploaded: uploadedRequired,
  };
}

export function getSuggestedDocuments(assetType: string): DocumentRequirement[] {
  const requirements = IrishDocumentRequirements[assetType] || [];
